    ...
    <div className="relative z-50">
      <form
        ...
        <input
          ...
          className="text-xs w-[20.rem] h-6 px-2 py-1 bg-[hsl(214,13%,47%)] rounded-lg text-gray-300 text-center"></input>
        <button
          type="submit"
          className="text-xs bg-slate-900 text-slate-300 w-[2.5rem] h-6 px-2 py-2 rounded-r-lg text-center absolute right-0 hover:bg-slate-700">
          검색
        </button>
      </form>
      ...
      {checkEqualName(searchTerm).length > 0 && (
        <div
          ...
          <div
            className={`w-0 bottom-0 h-0 border-x-transparent border-x-8 border-b-[8px] border-gray-700 -translate-y-1/2`}></div>
          <ul
            className={`w-40 max-h-[134px] bg-gray-700 rounded-lg absolute top-0 overflow-auto scrollbar-none`}>
